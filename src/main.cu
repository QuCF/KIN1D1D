...
void kin_outgoing_form1(int argc, char *argv[])
{
    ...
    // ---
    qreal Tref = 1.0e4 * cc.ev_; // in erg
    qreal den_ref = 1e12; // in cm-3

    uint Lx = 100;  // spatial size
    uint Lv = 4;  // velocity size
    qreal source_x0 = 50.;  // source position 
    qreal source_ds = 1.; // source width
    qreal diff = -0.002;
    string folder_to_save = "../../results/KIN1D1D-results/diff0002";
    // string folder_to_save = "../../results/KIN1D1D-results/diff0";

    // uint Lx = 50;  // spatial size
    // uint Lv = 4;  // velocity size
    // qreal source_x0 = 25.;  // source position 
    // qreal source_ds = 1.; // source width
    // qreal diff = -0.002;
    ...
    // // --- to find the spectral norm and the condition number of the matrix ---
    // dd.svd();


    // --- to solve the A x = b system ---
    dd.save_matrices();
