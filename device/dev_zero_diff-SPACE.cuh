...
*/
__global__ void Diff_ZERO_set_sparse_matrix_rows()
{
    ...
    if(ix == 0) 
    {
        if(iv < Nvh)
            ...
            id_r1 = (Nd+1+1) * Nvh + (iv - Nvh);
    }
    else if(ix == Nx-1) 
    {
        if(iv < Nvh)
            id_r1 = sh_ix0 + sh_bulk + iv; 
        else
            id_r1 = sh_ix0 + sh_bulk + dev_dd_.Nv_h + (Nd+1+1)*(iv - Nvh);
    }
    else
    {
...
__global__ void Diff_ZERO_set_matrix_values()
{
    ...
    else if(ix == Nx-1) 
    {
        ...
        {
            sh = sh_ix0 + sh_bulk + Nvh + (Nd+1+1)*(iv - Nvh);
